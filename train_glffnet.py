...
from models import GLFFNet
from torch.utils.data import DataLoader
from datasets import *
import argparse
from loss import CompactCenterLoss

def train(train_loader, net, criterion, ccl_criterion, optimizer, epoch):
    """
    ...
    for i, (views, pcs, labels) in enumerate(train_loader):
        ...
        ccl_input = torch.cat([fts])
        ccl_target = torch.cat([labels])
        ccl_loss = ccl_criterion(ccl_input, ccl_target)[0]

        loss = ce_loss + 0.5*ccl_loss

        prec.add(preds.detach(), labels.detach())
        losses.add(ce_loss.item())
        losses.add(ccl_loss.item())
...
def validate(val_loader, net, epoch):
    ...
    print(f' mean class accuracy at epoch {epoch}: {(np.mean(np.array(total_right_class)/np.array(total_seen_class,dtype=float)))} ')
    print(f' map at epoch {epoch}: {mAP} ')
    return prec.value(1), mAP


def save_ckpt(epoch, epoch_pc, epoch_all, best_prec1, net, optimizer_pc, optimizer_all, training_conf=config.glff_net):
    ckpt = dict(
        epoch=epoch,
        epoch_pc=epoch_pc,
        epoch_all=epoch_all,
    ...
    torch.save(ckpt, config.glff_net.ckpt_file)
...
def main():
    ...
    total_batch_sz = config.glff_net.train.batch_sz * len(config.available_gpus.split(','))
    total_epoch = config.glff_net.train.max_epoch

    if args.gpu is not None:
        os.environ['CUDA_VISIBLE_DEVICES'] = args.gpu
        total_batch_sz= config.glff_net.train.batch_sz * len(args.gpu.split(','))
    if args.epochs is not None:
        total_epoch = args.epochs


    train_dataset = pc_view_data(config.glff_net.pc_root,
                                 config.glff_net.view_root,
                                 status=STATUS_TRAIN,
                                 base_model_name=config.base_model_name)
    val_dataset = pc_view_data(config.glff_net.pc_root,
                               config.glff_net.view_root,
                               status=STATUS_TEST,
                               base_model_name=config.base_model_name)
    ...
    if config.glff_net.train.optim == 'Adam':
        optimizer_fc = optim.Adam(fc_param, config.glff_net.train.fc_lr,
                                  weight_decay=config.glff_net.train.weight_decay)

        optimizer_all = optim.Adam(net.parameters(), config.glff_net.train.all_lr,
                                   weight_decay=config.glff_net.train.weight_decay)
    elif config.glff_net.train.optim == 'SGD':
        optimizer_fc = optim.SGD(fc_param, config.glff_net.train.fc_lr,
                                 momentum=config.glff_net.train.momentum,
                                 weight_decay=config.glff_net.train.weight_decay)

        optimizer_all = optim.SGD(net.parameters(), config.glff_net.train.all_lr,
                                  momentum=config.glff_net.train.momentum,
                                  weight_decay=config.glff_net.train.weight_decay)
    else:
        raise NotImplementedError
    print(f'use {config.glff_net.train.optim} optimizer')
    print(f'Sclae:{net.module.n_scale} ')


    if config.glff_net.train.resume:
        print(f'loading pretrained model from {config.glff_net.ckpt_file}')
        checkpoint = torch.load(config.glff_net.ckpt_file)
        ...
        if config.glff_net.train.resume_epoch is not None:
            resume_epoch = config.glff_net.train.resume_epoch
        else:
            resume_epoch = max(checkpoint['epoch_pc'], checkpoint['epoch_all'])

    if config.glff_net.train.iter_train == False:
        print ('No iter')
    ...
    # ccl_loss
    ccl_criterion = CompactCenterLoss(margin=5, num_classes=40,feat_dim=256)
    ccl_criterion = ccl_criterion.to(device=config.device)


    for epoch in range(resume_epoch, total_epoch):


        if config.glff_net.train.iter_train == True:
            if epoch < 12:
                ...
                train(train_loader, net, criterion, ccl_criterion, optimizer_fc, epoch)
                epoch_pc += 1
                ...
                train(train_loader, net, criterion, ccl_criterion, optimizer_all, epoch)
                epoch_pc_view += 1
